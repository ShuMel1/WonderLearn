...
object AppStrings {
  ...
  val level_complete = LocalizedString(mapOf(Language.ARMENIAN to "Ապրե՛ս", Language.ENGLISH to "Great job!", Language.RUSSIAN to "Молодец!"))
  val level_next = LocalizedString(mapOf(Language.ARMENIAN to "Հաջորդը", Language.ENGLISH to "Next", Language.RUSSIAN to "Дальше"))
  val level_repeat = LocalizedString(mapOf(Language.ARMENIAN to "Կրկնել", Language.ENGLISH to "Repeat", Language.RUSSIAN to "Ещё раз"))
  val level_home = LocalizedString(mapOf(Language.ARMENIAN to "Տուն", Language.ENGLISH to "Home", Language.RUSSIAN to "Домой"))
  ...
  val lock_explain = LocalizedString(mapOf(Language.ARMENIAN to "Էկրանը կմնա այս հավելվածում երեխայի համար։ Սեղմած պահիր կոճակը՝ ապակողպելու համար։", Language.ENGLISH to "The screen will stay on this app for your child. Hold the button to unlock.", Language.RUSSIAN to "Экран останется на этом приложении для ребёнка. Удерживай кнопку, чтобы разблокировать."))
  val lock_hold_to_unlock = LocalizedString(mapOf(Language.ARMENIAN to "Սեղմած պահիր՝ ապակողպելու համար", Language.ENGLISH to "Hold to unlock", Language.RUSSIAN to "Удерживай, чтобы разблокировать"))
